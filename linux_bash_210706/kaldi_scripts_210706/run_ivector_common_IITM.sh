...
nj=50
data_folder=data
exp_folder=exp_augment_4gram_srilm
train_set=train_Augment/train_aug  # you might set this to e.g. train.
test_sets="dev_NPTEL dev_IITM" 
gmm=tri3_7000_140000                 # This specifies a GMM-dir from the features of the type you're training the system on;
                         # it should contain alignments for 'train_set'.
exp_old_folder=exp_replicate_4gram_srilm
num_threads_ubm=32
...
gmm_dir=$exp_old_folder/${gmm}
ali_dir=$exp_old_folder/${gmm}_ali_${train_set}_sp
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ...
    $exp_old_folder/nnet3${nnet3_affix}/extractor $ivectordir
  ...
  for data in ${test_sets}; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "${nspk_nj}" \
      $data_folder/${data}_hires $exp_old_folder/nnet3${nnet3_affix}/extractor \
