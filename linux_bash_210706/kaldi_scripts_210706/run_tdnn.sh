...
stage=0  #0
nj=50
tag=augment_4gram_srilm  #experiment tag, so that models are not overwritten; make sure change this if running a new experiment.
expdir=exp_${tag} 
datadir=data #Give the absolute path in case your data directory is not present in the current working directory
train_set=train_Augment/train_aug        #Train set Name
test_sets="dev_NPTEL dev_IITM"        #"test_dev93 test_eval92"
#The best TRI3 MODEL
gmm=tri3_7000_140000  #tri4b # this is the source gmm-dir that we'll use for alignments; it
                            # should have alignments for the specified training data.

num_threads_ubm=30
expolddir=exp_replicate_4gram_srilm
nj_extractor=50
...
affix=1g   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuriation.
common_egs_dir= 
reporting_email=
...
gmm_dir=$expolddir/${gmm}
ali_dir=$expolddir/${gmm}_ali_${train_set}_sp
...
original_lang=lang_4gram_srilm
...
if [ $stage -le 12 ]; then
	...
	if [ -d $lang ]; then
		if [ $lang/L.fst -nt $datadir/lang/L.fst ]; then
	 	...
	 	cp -r $datadir/$original_lang $lang
...
if [ $stage -le 13 ]; then
	# Get the alignments as lattices (gives the chain training more freedom).
  	...
  	steps/align_fmllr_lats.sh --nj 20 --cmd "$train_cmd" ${lores_train_data_dir} \
    	$datadir/$original_lang $gmm_dir $lat_dir
