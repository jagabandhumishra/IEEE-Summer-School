...
if [ $stage -le 17 ]; then
  ...
  utils/lang/check_phones_compatible.sh \
    data/lang_4gram_srilm/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --self-loop-scale 1.0 data/lang_4gram_srilm \
    $tree_dir $tree_dir/graph_LSTM || exit 1;
...
if [ $stage -le 18 ]; then
 ...
 #     for lmtype in tgpr bd_tgpr; do
        steps/nnet3/decode.sh \
          ...
          $tree_dir/graph_LSTM data/${data}_hires ${dir}/decode_${lmtype}_${data_affix} || exit 1
  #    done
  #    steps/lmrescore.sh \
  #      --self-loop-scale 1.0 \
